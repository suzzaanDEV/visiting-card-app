...
const EnhancedChart = ({ data, title, type = 'line', color = 'emerald', height = 'h-64' }) => {
  if (!data || data.length === 0) {
    return (
      <div className={`${height} flex items-center justify-center bg-gray-50 dark:bg-slate-900 rounded-lg`}>
        <div className="text-center">
          <FiBarChart2 className="h-12 w-12 text-gray-400 dark:text-slate-500 mx-auto mb-3" />
...
const AnalyticsDashboardNew = () => {
  ...
  const StatCard = ({ title, value, change, icon: Icon, color, subtitle, loading = false }) => (
    <motion.div
      ...
      className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 hover:shadow-md transition-all duration-200 group"
    >
  ...
  const ActivityItem = ({ activity }) => {
    ...
    const getActivityColor = (type) => {
      switch (type) {
        case 'card_created': return 'text-emerald-600 bg-emerald-100';
        case 'card_viewed': return 'text-green-600 bg-green-100';
        case 'card_shared': return 'text-green-600 bg-green-100';
        case 'user_registered': return 'text-orange-600 bg-orange-100';
        case 'card_loved': return 'text-green-600 bg-green-100';
        case 'card_downloaded': return 'text-emerald-600 bg-emerald-100';
  ...
  const TopCardItem = ({ card, rank }) => (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className="flex items-center justify-between p-3 hover:bg-gray-50 dark:bg-slate-900 dark:hover:bg-slate-700 rounded-lg transition-colors"
    >
      <div className="flex items-center space-x-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-gray-900 dark:text-slate-100 truncate">{card.name}</p>
    ...
    <AdminLayout title="Analytics Dashboard">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 dark:text-slate-400">Detailed insights and performance metrics</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 mb-8">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-semibold dark:text-slate-100">{analytics.deviceAnalytics?.desktop || 0}%</span>
            </div>
            <div className="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2">
              <div className="bg-emerald-500 h-2 rounded-full" style={{ width: `${analytics.deviceAnalytics?.desktop || 0}%` }}></div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-semibold dark:text-slate-100">{analytics.deviceAnalytics?.mobile || 0}%</span>
            </div>
            <div className="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-semibold dark:text-slate-100">{analytics.deviceAnalytics?.tablet || 0}%</span>
            </div>
            <div className="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2">
              <div className="bg-green-500 h-2 rounded-full" style={{ width: `${analytics.deviceAnalytics?.tablet || 0}%` }}></div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100 mb-4">Geographic Distribution</h3>
          <div className="space-y-3">
            {Object.entries(analytics.geographicAnalytics || {}).map(([country, percentage]) => (
              <div key={country} className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <FiMapPin className="h-4 w-4 text-gray-400 dark:text-slate-500" />
                  <span className="text-sm text-gray-700 dark:text-slate-300">{country}</span>
                </div>
                <span className="text-sm font-semibold dark:text-slate-100">{percentage}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-semibold dark:text-slate-100">{analytics.engagementMetrics?.avgSessionTime || 0} min</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-700 dark:text-slate-300">Bounce Rate</span>
              <span className="text-sm font-semibold dark:text-slate-100">{analytics.engagementMetrics?.bounceRate || 0}%</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-700 dark:text-slate-300">Growth Rate</span>
              <span className="text-sm font-semibold text-green-600 dark:text-green-400">+{analytics.overview?.growthRate || 0}%</span>
            </div>
          </div>
        </div>
      </div>
