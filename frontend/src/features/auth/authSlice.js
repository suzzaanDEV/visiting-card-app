...
const initialState = {
  ...
  twoFactor: null, // temporary storage for 2FA flows
};
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(authThunks.login.pending, (state) => {
        ...
        state.twoFactor = null; // Clear two-factor flow on new attempt
      })
      .addCase(authThunks.login.fulfilled, (state, action) => {
        ...
        if (action.payload?.twoFactor) {
          ...
          state.twoFactor = {
            ...
            devOtp: action.payload.devOtp,
            otpMessage: action.payload.otpMessage,
          };
        } else {
          state.isAuthenticated = true;
      ...
      .addCase(authThunks.login.rejected, (state, action) => {
        ...
        state.twoFactor = null; // Ensure no partial 2FA state on error
      ...
      .addCase(authThunks.verifyTwoFactor.pending, (state) => {
        state.isLoading = true;
        state.error = null;
      })
      .addCase(authThunks.verifyTwoFactor.fulfilled, (state, action) => {
        state.isLoading = false;
        state.isAuthenticated = true;
        state.user = action.payload;
        state.twoFactor = null;
      ...
      .addCase(authThunks.verifyTwoFactor.rejected, (state, action) => {
        state.isLoading = false;
      ...
      // --- Upload Avatar Thunk ---
      .addCase(authThunks.uploadAvatar.pending, (state) => {
        state.isLoading = true;
        state.error = null;
      })
      .addCase(authThunks.uploadAvatar.fulfilled, (state, action) => {
        state.isLoading = false;
        state.user = action.payload;
        state.error = null;
      })
      .addCase(authThunks.uploadAvatar.rejected, (state, action) => {
        state.isLoading = false;
        state.error = action.payload || 'Failed to upload avatar';
      })

      // --- Remove Avatar Thunk ---
      .addCase(authThunks.removeAvatar.pending, (state) => {
        state.isLoading = true;
        state.error = null;
      })
      .addCase(authThunks.removeAvatar.fulfilled, (state, action) => {
        state.isLoading = false;
        state.user = action.payload;
        state.error = null;
      })
      .addCase(authThunks.removeAvatar.rejected, (state, action) => {
        state.isLoading = false;
        state.error = action.payload || 'Failed to remove avatar';
      })
...
export const { clearAuthError, updateUserProfileLocal, setInitialized, hydrateFromStorage, clearTwoFactor } = authSlice.actions;
